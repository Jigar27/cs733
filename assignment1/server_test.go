...
func TestTCPSimple(t *testing.T) {
	...
	check(t, arr[0], "CONTENTS")
	check(t, arr[1], fmt.Sprintf("%v", version)) // expect only accepts strings, convert int version to string
	check(t, arr[2], fmt.Sprintf("%v", len(contents)))	
	scanner.Scan()
	check(t, contents, scanner.Text())
...
func client(t *testing.T, filename string, content string, ack chan int64){
	...
	Writing(t,conn,"write "+filename+" 7 5000")
	...
	Writing(t, conn, "cas "+filename+" "+reply[1]+" 5000")
	Writing(t, conn, "content")
