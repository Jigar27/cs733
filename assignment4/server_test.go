...
var servers map[int]*exec.Cmd
var errNoConn = errors.New("Connection is closed")

func startServers(id int) (err error) {
	servers[id] = exec.Command("sh", "-c", "server " + strconv.Itoa(id))
	servers[id].Stdout = os.Stdout
	servers[id].Stderr = os.Stderr
	err = servers[id].Start()
	return err
}
...
func runServer() {
	...
	command = exec.Command("sh","-c","go install server.go raftnode.go statemachine.go")
...
*/
	runServer()
	//checkErr(t, err, "TestNormalOperation: runServer")
	err = startCluster()
	checkErr(t, err, "TestNormalOperation: startcluster")
...
func mkClient(t *testing.T, port int) *Client {
	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", "localhost:"+strconv.Itoa(port))
	if err == nil {
